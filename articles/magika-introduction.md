---
title: "AI の力でファイル形式を判定する Google 製の「Magika」を試してみる"
emoji: "🪄"
type: "tech" # tech: 技術記事 / idea: アイデア
topics: ["cli", "python", "ai"]
published: true
published_at: 2024-02-17 18:00
---
...
- [検証したバージョン](#検証したバージョン)
- [Magika の使い方](#magika-の使い方)
  - [インストール](#インストール)
  - [CLI として使う](#cli-として使う)
  - [Python ライブラリとして使う](#python-ライブラリとして使う)
  - [JavaScript ライブラリとして使う](#javascript-ライブラリとして使う)
  - [Magika がサポートしているファイル形式](#magika-がサポートしているファイル形式)
- [まとめ](#まとめ)
...
- [単一のファイルの形式を判定する](#単一のファイルの形式を判定する)
- [複数のファイルの形式を判定する](#複数のファイルの形式を判定する)
- [判定するファイルを標準入力から読み取る](#判定するファイルを標準入力から読み取る)
- [JSON 形式で出力する](#json-形式で出力する)

### 単一のファイルの形式を判定する

`magika` にファイル名を渡して実行すると、そのファイルの形式を判定してくれます。

```sh
$ magika <ファイル名>
```

例えば、次のようなファイルを `main.go` という名前で作成します。
...
`-r`, `--recursive` フラグを使うと、指定したディレクトリを再帰的に探索してファイル形式を判定することができます。
...
標準入力から読み取ることもできます。
この場合、 `magika` の引数には `-` を指定する必要があります。
...
`--json` フラグをつけると、スコアなど様々な情報を含めた JSON 形式で出力してくれます。
...
- [単一のファイルの形式を判定する](#単一のファイルの形式を判定する-1)
- [複数のファイルの形式を判定する](#複数のファイルの形式を判定する-1)
- [バイト列を渡して判定する](#バイト列を渡して判定する)

### 単一のファイルの形式を判定する

`Magika` オブジェクトの `identify_path` メソッドに `Path` オブジェクトを渡すことで、単一のファイルの形式を判定できます。
...
複数のファイルの形式を判定するためには `identify_paths` メソッドを使います。
...
`MagikaResult` オブジェクトのリストが返ってきます。
...
`identify_bytes` メソッドを使うとバイト列を渡してファイル形式を判定できます。
...
なお、大きなファイルを扱う場合は `identify_path` か `identify_bytes` を使った方が基本的にはいいみたいです。
...
> > https://github.com/google/magika/blob/main/python/DOCS.md
...
https://www.npmjs.com/package/magika

こちらは[デモページ](https://google.github.io/magika/)用に開発されたとのことです。

## Magika がサポートしているファイル形式

`--list-output-content-types` フラグをつけて実行すると Magika がサポートしているファイル形式の一覧を確認できます。

```sh
$ magika --list-output-content-types
```

もしくは [magika/docs/supported-content-types-list.md](https://github.com/google/magika/blob/main/docs/supported-content-types-list.md) をご参照ください。
